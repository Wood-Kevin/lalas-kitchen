...
- `calculateCascades(board)`: after a clear, drops pieces down to fill gaps, spawns new pieces at the top. On a non-rectangular board it works **per column segment** — a `'void'` cell is a fixed floor, so pieces compact within each maximal non-void run and refill from that segment's own top, never falling through or past a void (see the board-shape / void-cell entry)
...
Board dimensions and objectives live per level, not in this skin file, since board shape is a source of variety across levels, not a skin-wide constant. "Board shape" originally meant only per-level rectangular `rows`/`cols`; genuine **non-rectangular** shapes are now a level-content choice too, via the optional `LevelConfig.voidCells` (a list of hole coordinates rendered as fixed `'void'` cells — see the board-shape / void-cell entry in `engine/DECISIONS.md` and the plus-shaped Level 4 showcase).
...
- Special piece behaviors beyond blockers, the striped row/column clearer, the color bomb, the special-piece combos, and the area bomb (all built: the striped piece; the color bomb — a straight 5-run that detonates every piece of one type on swap; the striped+striped cross and striped+bomb super-combos; and the area bomb — a 2×2 square that spawns a colorless bomb clearing a 3×3 when swapped; see `engine/DECISIONS.md`'s striped-piece, color-bomb, special-piece combos, and area-bomb entries; blocker clearing was built in Phase 6, below; sweep/blast/detonation **chaining** — a special caught in another's sweep/detonation/blast/combo-clear firing its own effect, recursively — was brought into scope and built, see `engine/DECISIONS.md`'s special-piece-chaining entry and `docs/verification/special-piece-chaining/`). Still deferred: `area + special` combos (an area bomb swapped *directly into* another special — currently a snap-back; distinct from chaining, which is a special *caught in a clear*), L/T-shape triggers (including an L/T-formed color bomb or area bomb, vs. the built straight-5 and pure-2×2 triggers), and the per-link chaining/convert-to-striped animation flash (the settled clear is computed directly, with no staged intermediate frame) — no longer the whole "row clearers" line it used to be.
...
- Non-rectangular board shapes beyond the built void-cell mechanic. A `'void'` sentinel piece makes cells non-playable (segmented gravity, void-aware matching/legal-moves/shuffle/generator/render, `LevelConfig.voidCells`), showcased by the hand-built plus-shaped Level 4 — built this session (see the board-shape / void-cell entry in `engine/DECISIONS.md` and `docs/verification/board-shape/`). Still deferred: generator-driven shapes (every shape is hand-authored today) and void × specials/denial-spread interaction beyond "an inert fixed hole." See `DEFERRED_COMPLEXITY.md`.
