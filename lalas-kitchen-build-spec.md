...
- Special piece behaviors beyond blockers, the striped row/column clearer, the color bomb, the special-piece combos, and the area bomb (all built: the striped piece; the color bomb — a straight 5-run that detonates every piece of one type on swap; the striped+striped cross and striped+bomb super-combos; and the area bomb — a 2×2 square that clears a 3×3 when matched; see `engine/DECISIONS.md`'s striped-piece, color-bomb, special-piece combos, and area-bomb entries; blocker clearing was built in Phase 6, below). Still deferred: sweep/blast/detonation chaining, and L/T-shape triggers (including an L/T-formed color bomb or area bomb, vs. the built straight-5 and pure-2×2 triggers) — no longer the whole "row clearers" line it used to be.
