...
**Build:**
- Board representation as a 2D array of piece objects: `{ id: string, type: 'normal' | 'striped' | 'blocker', matchType?: string }` (the `'striped'` member started as a `'row_clearer'` placeholder and was built out later — see `engine/DECISIONS.md`'s striped-piece entry; blockers also carry `hitsRemaining`, striped pieces a `direction`)
...
- Special piece behaviors beyond blockers and the now-built striped row/column clearer (built in the striped-piece session — see `engine/DECISIONS.md`'s striped-piece entry; blocker clearing was built in Phase 6, below). Still deferred: sweep chaining, 5+-match specials, and L/T-shape combos — no longer the whole "row clearers" line it used to be.
