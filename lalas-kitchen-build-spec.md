...
- Special piece behaviors beyond blockers, the striped row/column clearer, and the color bomb (the striped piece and the color bomb — a straight 5-run that detonates every piece of one type on swap — are both built; see `engine/DECISIONS.md`'s striped-piece and color-bomb entries; blocker clearing was built in Phase 6, below). Still deferred: sweep/detonation chaining, L/T-shape triggers (including an L/T-formed color bomb, vs. the built straight-5 trigger), and bomb+striped super-combos — no longer the whole "row clearers" line it used to be.
