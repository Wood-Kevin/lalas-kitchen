...
- Special piece behavior beyond blockers and the now-built striped piece (the striped row/column clearer was brought into scope and built — see the Data Model Notes above and `engine/DECISIONS.md`'s striped-piece entry; blocker clearing was built earlier, see the Phase 6 section). Still deferred: sweep chaining, 5+-match specials (color bombs), and L/T-shape combos.
