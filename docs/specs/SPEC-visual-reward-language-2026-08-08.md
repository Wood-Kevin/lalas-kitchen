...
> **Correction (2026-08-08, discovered building the following HUD spec):** the claim above — "only the whole-move total reaches `ApplyMoveResult`" — was imprecise. The actual state at the time this spec was written: **no score value reached `ApplyMoveResult` at all**, not even the whole-move total; `scoreGained` was consumed entirely internally to update a `'score'`-type `Objective.currentCount`, then discarded for every other objective type. This didn't change the DECISION here (reward-intensity stays presentation-only, correctly derived from the diff, not from real score) — it only means the "no per-pass breakdown exists" framing understated the actual gap. `ApplyMoveResult.score` was added for real in the HUD reward-texture-and-character spec that follows this one.
